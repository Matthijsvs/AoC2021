digits = """gabfed acbdfg cd gebcd gdfecb bgfdcae ecdf dcb gdbfe gaceb | dcb dgfceab cbfdeg edcf
...
class solver():
    def __init__(self,input):
        ...
        self.mapping = {}
    def solve(self):
        special = {2:1,3:7,4:4,7:8}
        for i in self.data:
            l = len(i)
            if l in special:
                   self.mapping[special[l]]=set(i)
        for i in self.data:
            print("-----------------------",i,"--------------------------------")
            l = len(i)
            s = set(i)
            if l in special:
                ...
                if len(s&self.mapping[7]) == 3:
                    self.mapping[3]=s
                elif len(s&self.mapping[4]) == 3:
                    self.mapping[5] = s
                else: 
                    self.mapping[2] = s
            elif l == 6:
                if len(s&self.mapping[4]) == 4:
                    self.mapping[9]=s
                elif len(s&self.mapping[7]) == 3:
                    self.mapping[0] = s
                else: 
                    self.mapping[6] = s
            else:
...
for line in digits.splitlines():
    g = 0
    pin,out = line.split(" | ")
    l = map(sorted,pin.split(" "))
    l = list(l)
    c = solver(l)
    c.solve()
    l = map(sorted,out.split(" "))
    l = list(l)
    for item in l:
        g = g*10+c.get(set(item))
    s+=g
print(s)
